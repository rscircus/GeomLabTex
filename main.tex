...
It is NP-hard to decide, whether for a given set $D$ of disks with boundary points $P_i$ there is a physical realization $\Pi$ (with rotation), s.t. the 

\[max_{p\in\bigcup P_i} dist_\Pi(p) \geq k\].

\subsubsection*{Proof}
The proof is inspired and in large parts analogous to [Cabello, Speckmann, Haverkort].
...
We use the three disk types in Figures 1 and 2 to construct for each variable a so-called "gadget". A gadget is a group of neutral disks forming a circle, each overlapping the next by a fraction of 1/6. Notice that the maximum possible distance of separation points to occluded area is 15 degrees. The gadget is constructed in such a way that it can only be oriented clockwise or counter-clockwise if we want $k>0$ distance to the occluded area. Figure 3 depicts a prototype of a gadget, all the disks are of type one for now. In reality the number of disks used will be higher because the curvature cannot be this high.
...
\begin{figure}[h]
\centering
\includegraphics[width=10cm, height=5cm]{assets/np-hardness/negative.png}
\caption{A "true" gadget with a "false" disk}
\end{figure}
...
\section{Stuff left to do}
1. Less talking, better figures and drawings.
2. Deal with non-real-RAM (at present, centerpoints may be irrational)
3. Deal with "wiggle-space". The precise rotation of the disks is not fixed, there is some interval of possible rotation. We need to convince ourselves that this will not enable unwanted behaviour of the pie-charts that might break the logic.
